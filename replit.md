...
- Tout le site affiche désormais ses prix en **HTG uniquement** (voir section "Devise" ci-dessous), donc le montant envoyé à Pay'm Plop plop correspond exactement à ce qui est affiché — plus de conversion à faire.
- Vérifié end-to-end (12 juillet 2026) : création de transaction réelle (retourne une URL MonCash valide), polling de statut (`trans_status: "no"` avant paiement), rejet correct des montants invalides (<20 HTG), et gestion des erreurs (refference_id manquant/inconnu).

## Devise — tout en gourdes (HTG)
- Tous les prix du site (boutique, panier, paiement, confirmation, devis) sont affichés en **HTG uniquement**, plus aucun **€** nulle part.
- Catalogue produits (`data/produits.json`, géré depuis `dashboard.html`) : prix par défaut plafonnés à **10 000 HTG** par produit. Le champ prix du formulaire admin a un `max="10000"` côté client, et `server.js` (`POST`/`PUT /api/produits`) clamp aussi la valeur à [0, 10000] côté serveur par sécurité.
- Les articles de démo statiques (`panier.html`, `paiement.html`, `confirmation.html`) ont été recalculés en HTG cohérents avec ce plafond.
- Les tranches de budget de `devis.html` (demande de devis, pas un prix produit) restent à une échelle plus large en HTG puisqu'elles couvrent des projets complets, pas un article du catalogue.

## Redirection après connexion
- La page d'accueil (`index.html`) reste la vitrine marketing pour les visiteurs non connectés (hero, "Demander un devis"...).
- Un client déjà connecté qui arrive sur `/` est automatiquement redirigé vers `/boutique.html` (`public/auth.js`, fonction `updateHomepage`) — il ne revoit plus jamais la page marketing "logged-out" une fois authentifié.
- Après connexion réussie sur `login.html`, un client (non-admin) est envoyé vers `/boutique.html` par défaut (au lieu de `/`), sauf si un paramètre `?next=` précisait déjà une autre destination. Les admins continuent d'être envoyés vers `/dashboard.html`.
